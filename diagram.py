...
###
class Graph:
	""" NX Graph class with nx graph attribute
	"""
	...
	def PopulateGraph(self):
		...
		### degree of graph
		### http://stackoverflow.com/questions/16566871/node-size-dependent-on-the-node-degree-on-networkx
		d = nx.degree(self.graph)

		### odes size
		node_sizes = [v * 100 for v in d.values()] #[ 800 for node in nodes]
		...
		### draw activity edge label
 		edge_labels=dict([((u,v,),round(self.getActivity(u)+self.getActivity(v),3)) for u,v,d in self.graph.edges(data=True)])
 	 	self.edge_collection = nx.draw_networkx_edge_labels(self.graph,pos,edge_labels=edge_labels)

	def GetPos(self):
		#try:
		#	pos=nx.graphviz_layout(self.graph, prog='dot')
		#except ValueError:
		### you can use the parameter scale to increase the global distance between the nodes.
		pos=nx.spring_layout(self.graph, scale=5, iterations=50)
	...
	def OnUpdate(self, evt):
		...
		self.fig.canvas.restore_region(self.background)
 		...
 		if hasattr(self, 'fig'):
			...
			### draw new weight
			e = nx.draw_networkx_edge_labels(self.graph,self.GetPos(),edge_labels=edge_labels)

#			self.edge_collection=e

			# just draw the animated artist
#			self.ax.draw_artist(self.edge_collection)
...
# 	###-------------------------------
	...
	canvas.set_window_title("%s - Abstract simulator three"%dia.label)
